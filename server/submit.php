...
$uploadsDir = $paths['public'] . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'photos';
if (!is_dir($uploadsDir)) {
  @mkdir($uploadsDir, 0775, true);
}

$docsDir = $paths['public'] . DIRECTORY_SEPARATOR . 'uploads' . DIRECTORY_SEPARATOR . 'docs';
if (!is_dir($docsDir)) {
  @mkdir($docsDir, 0775, true);
}

function upfile($field, $destDir, $publicRoot) {
  ...
  $rel = str_replace($publicRoot, '', $target);
  $rel = str_replace('\\', '/', $rel);
  return $rel;
}

$photoPath = upfile('photo', $uploadsDir, $paths['public']);
$certPath = upfile('certification', $docsDir, $paths['public']);
$payload = [
