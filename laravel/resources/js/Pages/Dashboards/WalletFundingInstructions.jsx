...
export default function WalletFundingInstructions({ intent, bank }) {
    const flash = usePage().props.flash;
    const [data, setData] = useState({
        payment_reference: '',
        amount_naira: Number((intent?.amount_kobo || 0) / 100).toFixed(0),
        receipt: null,
    });
    const [errors, setErrors] = useState({});
    const [processing, setProcessing] = useState(false);

    const amountNaira = useMemo(() => Math.floor(Number(intent?.amount_kobo || 0) / 100), [intent?.amount_kobo]);
    ...
    const submit = (e) => {
        ...
        router.post(route('wallet.funding.submit', intent.id), form, {
            forceFormData: true,
    ...
    return (
        <DashboardLayout title="Wallet top-up">
            <Head title="Wallet top-up" />
                    ...
                    <div className="dash-surface p-6">
                        ...
                        <div className="mt-6 space-y-3">
                            ...
                            <div className="dash-surface p-4">
                                ...
                                <div className="dash-title mt-2 text-xl font-bold">{intent?.reference}</div>
                            </div>
                        </div>
                    </div>
                </div>
